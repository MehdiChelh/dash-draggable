...
export ''_draggabledashboard

"""
    ''_draggabledashboard(;kwargs...)
    ''_draggabledashboard(children::Any;kwargs...)
    ''_draggabledashboard(children_maker::Function;kwargs...)
...
- `layout` (Array of Dicts; optional): Layout is a list(python)/vector(R) of dictionnary(Python)/list(R) with the format:
...
- `items_by_row` (Real; optional): (number) the default number of columns for an item with no predefined size.
The size of items can either be defined in the layout argument or in DashboardItem.
- `nrows` (Real; optional): (number) the default number of items by row.
Default is 2.
"""
function ''_draggabledashboard(; kwargs...)
        available_props = Symbol[:children, :id, :layout, :saveToClient, :clearLayoutOnClient, :items_by_row, :nrows]
        wild_props = Symbol[]
        return Component("''_draggabledashboard", "DraggableDashboard", "dash_draggable", available_props, wild_props; kwargs...)
end

''_draggabledashboard(children::Any; kwargs...) = ''_draggabledashboard(;kwargs..., children = children)
''_draggabledashboard(children_maker::Function; kwargs...) = ''_draggabledashboard(children_maker(); kwargs...)
